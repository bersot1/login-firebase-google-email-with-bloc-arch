...
class MockFireabaseAuth extends Mock implements firebase_auth.FirebaseAuth {}
...
void main() {
  ...
  group('AuthenticationRepository', () {
    ...
    setUp(() {
      ...
      firebaseAuth = MockFireabaseAuth();
    ...
    test('creates FirebaseAuth instance internally when not inject', () {
    ...
    group('SignUp', () {
      setUp(() {
        when(() => firebaseAuth.createUserWithEmailAndPassword(
            ...
            password: any(named: 'password'))).thenAnswer((_) => Future.value(MockUserCredential()));
      });
    ...
    group('loginWithGoogle', () {
      ...
      test('calls signIn Authentication and SignInWithCredentials', () async {
      ...
      test(
          'throws LoginInWithGoogleFailure and calls signIn authentication and '
      ...
      test('succeeds when signIn successds', () {
    ...
    group('logInWithEmailAndPassword', () {
      setUp(() {
        when(() =>
                firebaseAuth.signInWithEmailAndPassword(email: any(named: 'email'), password: any(named: 'password')))
            .thenAnswer(
          (_) => Future.value(
            MockUserCredential(),
          ),
      ...
      test('succeeds when signWithEmailAndPassword', () async {
        expect(authenticationRepository.logInWithEmailAndPassword(email: email, password: password), completes);
      });

      test(
          'throws LogInWithEmailAndPasswrodFailure '
          'when signInWithEmailAndPasswrod throws', () async {
        when(() => firebaseAuth.signInWithEmailAndPassword(email: email, password: password)).thenThrow(Exception());
        expect(authenticationRepository.logInWithEmailAndPassword(email: email, password: password),
    ...
    group('user', () {
      ...
      test('emits user when firebase user is not null', () async {
        ...
        await expectLater(authenticationRepository.user, emitsInOrder(const <UserModel>[user]));
    ...
    group('currentUser', () {
      test('returns UserModel.empty when cached user is null', () async {
        when(() => cache.read(key: AuthenticationRepository.userCacheKey)).thenReturn(null);

        expect(authenticationRepository.currentUser, equals(UserModel.empty));
      });

      test('returns UserModel when cached is not null', () async {
        when(() => cache.read<UserModel>(key: AuthenticationRepository.userCacheKey)).thenReturn(user);
